...
class BidsListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
                  ...
                  FutureBuilder<List<BidModel>>(
                    ...
                    builder: (ctx, snap) {
                      ...
                      return _buildBidList(
                          snap.data ?? [], formatter, userRole, db);
                    },
                  ),
              ...
              child: TabBarView(
                children: [
                  StreamBuilder<List<BidModel>>(
                    ...
                    builder: (ctx, snap) => _buildStreamContent(
                        snap, formatter, 'Received Offers', db, 'Farmer'),
                  ),
                  StreamBuilder<List<BidModel>>(
                    stream: db.streamBidsByBuyer(userId),
                    builder: (ctx, snap) => _buildStreamContent(
                        snap, formatter, 'My Placed Bids', db, 'Buyer'),
                  ),
                ],
              ),
            ),
    ...
    return StreamBuilder<List<BidModel>>(
      stream: db.streamBidsByBuyer(userId),
      builder: (ctx, snap) =>
          _buildStreamContent(snap, formatter, 'My Bids', db, userRole),
    );
  }

  Widget _buildStreamContent(AsyncSnapshot<List<BidModel>> snap,
      NumberFormat formatter, String title, DatabaseService db, String role) {
    if (snap.connectionState == ConnectionState.waiting) {
      return const Center(
  ...
  Widget _buildBidList(List<BidModel> bids, NumberFormat formatter, String role,
      DatabaseService db) {
    if (bids.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(role == 'Farmer' ? Icons.chat_bubble_outline : Icons.gavel,
                color: AppTheme.textMuted.withOpacity(0.3), size: 64),
            const SizedBox(height: 16),
            Text(role == 'Farmer' ? 'No messages yet' : 'No bids yet',
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('UGX ${formatter.format(bid.offeredPrice)}',
                                style: TextStyle(
                                    color: AppTheme.greenDark,
                                    fontSize: 18,
                                    fontWeight: FontWeight.w800)),
                            Text('${bid.productName} • Qty: ${bid.quantity}',
                                style: TextStyle(
                                    color: AppTheme.greenDark.withOpacity(0.7),
                                    fontSize: 12)),
                          ],
                        ),
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (role != 'Farmer' && role != 'Buyer')
                                  Text(
                                      'Buyer: ${bid.buyerPhone.isNotEmpty ? bid.buyerPhone : "No phone"}',
                                      style: TextStyle(
                          ...
                          if (canManage)
                            IconButton(
                              ...
                              onPressed: () => _showDirectMessageDialog(
                                  ctx, bid.buyerId, bid.buyerName, db),
                            ),
                        ],
                      ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                      'Farmer: ${bid.sellerPhone.isNotEmpty ? bid.sellerPhone : "No phone"}',
                                      style: TextStyle(
                                          color: AppTheme.textSecondary,
                                          fontSize: 12)),
                                ],
  ...
  void _showMessageDialog(
      BuildContext context, BidModel bid, DatabaseService db) {
    final tc = TextEditingController(text: bid.adminNotes ?? '');
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppTheme.surface,
        title: Text('Admin Note to Farmer',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              db.updateBidStatus(bid.id, bid.status,
                  adminNotes: tc.text.trim());
              Navigator.pop(ctx);
            },
  ...
  void _showDirectMessageDialog(BuildContext context, String recipientId,
      String recipientName, DatabaseService db) {
