  ...
  Widget build(BuildContext context) {
    return FutureBuilder<UserModel?>(
      ...
      builder: (context, userSnap) {
        ...
        return isAdmin ? _buildAdminMessages(currentUser) : _buildCustomerMessages(currentUser);
      },
    );
  }
    ...
    return StreamBuilder<List<MessageModel>>(
      ...
      builder: (context, snapshot) {
        ...
        for (final msg in allMessages) {
          final otherUserId = msg.senderId == 'support' ? msg.recipientId : msg.senderId;
        ...
        threads.sort((a, b) => b.value.first.createdAt.compareTo(a.value.first.createdAt));

        final totalUnreadCount = allMessages.where((m) => m.recipientId == 'support' && !m.isRead).length;
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('No customer chats yet', style: TextStyle(color: AppTheme.textSecondary, fontWeight: FontWeight.w500)),
                          ],
                        ),
                      )
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final userMsg = threadMessages.firstWhere((m) => m.senderId != 'support', orElse: () => latestMsg);

                          return FutureBuilder<UserModel?>(
                            future: _db.getUser(otherUserId),
                            builder: (context, userSnap) {
                              final user = userSnap.data;
                              final displayName = user?.name ?? userMsg.senderName;
                              final displayRole = user?.role ?? userMsg.senderRole;
                              ...
                              final threadUnreadCount = threadMessages.where((m) => m.recipientId == 'support' && !m.isRead).length;
                              final isThreadUnread = threadUnreadCount > 0;

                              return InkWell(
                                onTap: () async {
                                  // Mark support messages from this user as read
                                  await _db.markSupportMessagesRead(otherUserId);
                                  
                                  if (context.mounted) {
                                    // If user is loaded, push ChatScreen. Else fetch on tap.
                                    final targetUser = user ?? await _db.getUser(otherUserId);
                                    if (targetUser != null && context.mounted) {
                                      Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (_) => ChatScreen(currentUser: currentUser, otherUser: targetUser),
                                        ),
                                      );
                                    }
                                  }
                                ...
                                child: Container(
                                  color: isThreadUnread ? AppTheme.greenSurface.withOpacity(0.3) : Colors.transparent,
                                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                  child: Row(
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                Expanded(
                                                  child: Text(
                                                    ...
                                                    style: TextStyle(
                                                      ...
                                                      fontSize: 14,
                                                      fontWeight: isThreadUnread ? FontWeight.w700 : FontWeight.w500,
                                                    ),
                                                ...
                                                Text(
                                                  _formatTime(latestMsg.createdAt),
                                                  style: TextStyle(
                                                    color: isThreadUnread ? AppTheme.greenLight : AppTheme.textMuted,
                                                    fontSize: 11,
                                                    fontWeight: isThreadUnread ? FontWeight.w600 : FontWeight.normal,
                                                  ),
                                                ),
                                              ],
                                            ),
                                              ...
                                              children: [
                                                ...
                                                Expanded(
                                                  child: Text(
                                                    latestMsg.body,
                                                    style: TextStyle(
                                                      color: isThreadUnread ? AppTheme.textPrimary : AppTheme.textMuted,
                                                      fontSize: 12,
                                                      fontWeight: isThreadUnread ? FontWeight.w500 : FontWeight.normal,
                                                    ),
                                                    maxLines: 1,
                                                    overflow: TextOverflow.ellipsis,
                                                  ),
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            Icon(Icons.chat_bubble_outline, color: AppTheme.textMuted.withOpacity(0.3), size: 64),
                            const SizedBox(height: 16),
                            Text('No messages yet', style: TextStyle(color: AppTheme.textSecondary, fontWeight: FontWeight.w500)),
                          ...
                          return InkWell(
                            onTap: () {
                              ...
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => ChatScreen(currentUser: currentUser, otherUser: supportUser),
                                ),
                              );
                            },
                            child: Container(
