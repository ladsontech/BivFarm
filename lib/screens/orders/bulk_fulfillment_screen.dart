...
class _BulkFulfillmentScreenState extends State<BulkFulfillmentScreen> {
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.greenSurface, borderRadius: BorderRadius.circular(10)),
                  child: const Icon(Icons.call_split, color: AppTheme.greenDark),
                ),
          ...
          Expanded(
            child: StreamBuilder<List<ProductModel>>(
              ...
              builder: (ctx, snap) {
                ...
                var filtered = allProducts.where((p) => p.category == widget.order.category).toList();
                ...
                if (_selectedDistrict != null) {
                  filtered = filtered.where((p) => p.district == _selectedDistrict).toList();
                }
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _searchQuery.isNotEmpty || _selectedDistrict != null
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(p.productName, style: TextStyle(color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w600)),
                                  const SizedBox(height: 4),
                                  Text('Farmer: ${p.sellerName}', style: TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
                                  const SizedBox(height: 2),
                                  Text('Stock: ${p.quantity} ${p.quantityUnit}', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                                ],
                              ),
                            ),
                            ElevatedButton(
  ...
  void _showRequestDialog(ProductModel product) {
    final qtyCtrl = TextEditingController(text: widget.order.quantity.toString());
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('You are creating a split sub-request for the bulk order. The farmer will receive a standard bid notification from the Administrator.', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
            const SizedBox(height: 16),
            CustomTextField(label: 'Requested Quantity (${product.quantityUnit})', controller: qtyCtrl, keyboardType: TextInputType.number),
            const SizedBox(height: 12),
            CustomTextField(label: 'Offered Price (UGX)', controller: priceCtrl, keyboardType: TextInputType.number),
          ],
        ),
        actions: [
            ...
            onPressed: () async {
              ...
              try {
                ...
                final bid = BidModel(
                  ...
                  sellerPhone: 'Registered Farmer', // Fallback, not stored on ProductModel
                  buyerId: widget.order.buyerId, // The person who requested the bulk order
                  buyerName: 'Registry (Bulk Request)', // Mask the original buyer name
                  buyerPhone: 'Via Admin',
                  quantity: q,
                  offeredPrice: p,
                  status: 'Pending',
                  notes: 'Bulk fulfillment sub-request for Master Order #${widget.order.id.substring(0, 4)}',
                );
                
                await _db.addBid(bid);
                
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Supply requested to farmer successfully!')));
                }
              } catch (e) {
                if (mounted) {
